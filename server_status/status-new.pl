...
@temp=("hostname", "hostip",  "osname" , "ping","date","uptime", "loadavg","ramst","runningProcs", "diskst", "usertot", "lastst");
...
# my @temp =  qw(hostname hostip osname ping date uptime loadavg ramst runningProcs diskst usertot lastst ) ;
print '<thead><tr>';
    print "<tr>";
    foreach (@temp) {
        print "<th>$desc{$_}</th>";
